...
def search_restaurant(address, cuisine):
    ...
    if address:
        query = query.filter(
            Restaurant.Address.ilike(f"%{address}%")
        )

    if cuisine:
        query = query.filter(
            Restaurant.CuisineID == int(cuisine)
        )
    ...
    return [
        {
            ...
            "Opentime":
            r.Opentime.strftime("%H:%M")
            if r.Opentime else None,

            "Closetime":
            r.Closetime.strftime("%H:%M")
            if r.Closetime else None
        }
...
def get_history(user_id):
    ...
    for b in bookings:
        ...
        result.append({
            ...
            "BookingDate":
            b.BookingDate.strftime("%Y-%m-%d"),

            "BookingTime":
            b.BookingTime.strftime("%H:%M"),
            ...
            "RestaurantName":
            b.restaurant.RestaurantName
            if b.restaurant else None,

            "TableNumber":
            b.table.id
            if b.table else None
        })
